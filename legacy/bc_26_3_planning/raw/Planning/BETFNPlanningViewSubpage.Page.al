...
page 5138659 "BET FN Planning View Subpage"
{
    ...
    var
        ...
        SalesOrderAmountVisible_G, SalesOrderQtyVisible_G : Boolean;
        ColourStyle_G: Text;
    ...
    var
        BETFNPlanningLayoutTmplt: Record "BET FN Planning Layout Tmplt";
        LayoutTemplateCode: Code[20];
    begin
        //### Layout von Ebene oder von Beleg übernehmen:

        if PlanDoc_PT."Use Global Layout" then
            LayoutTemplateCode := PlanDoc_PT."Layout Template"
        else
            LayoutTemplateCode := PlanDocLevel_PT."Layout Template";


        if BETFNPlanningLayoutTmplt.Get(LayoutTemplateCode) then begin
            //### VK-Werte:
            GrossSalesAmountVisible_G := BETFNPlanningLayoutTmplt."Plan Sales Amount";          //### Bruttoumsatz
            DiscountVisible_G := BETFNPlanningLayoutTmplt."Plan Sales Discount";                //### Rabatt + Rabatt%
            SalesInclDiscountVisible_G := BETFNPlanningLayoutTmplt."Plan Sal. Am. incl. Discount";   //### Umsatz inkl. Rabatt
            SalesInitInvVisible_G := BETFNPlanningLayoutTmplt."Plan Sales Init. Inv.";          //### VK Anfangsbestand
            SalesInitClosingVisible_G := BETFNPlanningLayoutTmplt."Plan Sales Closing Inv.";    //### VK Endbestand
            ReductionVisible_G := BETFNPlanningLayoutTmplt."Plan Gross Sales Pr. Reduction";    //### Abschrift (Brutto-Preisabschrift)
            SalesAmtPurchVisible_G := BETFNPlanningLayoutTmplt."Plan Sales Am. Purchase";       //### VK Einkaufswert (Wareneingang)

            //### EK-Werte:
            CostOfSalesVisible_G := BETFNPlanningLayoutTmplt."Plan Cost of Sales";          //### EK Umsatz (=Wareneinsatz EK)
            CostAmtPurchVisible_G := BETFNPlanningLayoutTmplt."Plan Cost Am. Purchase";     //### EK WE (Limit)
            CostInitInvVisible_G := BETFNPlanningLayoutTmplt."Plan Cost Init. Inv.";        //### EK Anfangsbestand
            CostClosingInvVisible_G := BETFNPlanningLayoutTmplt."Plan Cost Closing Inv.";   //### EK Anfangsbestand

            //### Mengen:
            QtySalesVisible_G := BETFNPlanningLayoutTmplt."Plan Qty. Sale";                 //### Menge Verkauf
            QtyInitInvVisible_G := BETFNPlanningLayoutTmplt."Plan Qty. Init. Inv.";         //### Menge Anfangsbestand
            QtyClosingInvVisible_G := BETFNPlanningLayoutTmplt."Plan Qty. Closing Inv.";    //### Menge Endbestand
            QtyPurchVisible_G := BETFNPlanningLayoutTmplt."Plan Qty. Purchase";             //### Menge Einkauf

            PercentageVisible_G := BETFNPlanningLayoutTmplt."Plan Sales Percentage" and PlanDoc_PT."Auto Filter On Level Changing";

            SalesOrderAmountVisible_G := BETFNPlanningLayoutTmplt."Plan Sales Order Amount";
            SalesOrderQtyVisible_G := BETFNPlanningLayoutTmplt."Plan Sales Order Qty.";
        end;
